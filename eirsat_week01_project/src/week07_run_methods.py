...
def metric_row(run_id, fault_type, method, precision, recall, f1, mdd, fab, threshold):
    return {
        "run_id": run_id,
...
def main():
    ...
    for run_id, fault_type in RUNS.items():
        print(f"\nEvaluating {run_id} ({fault_type}) ...")
        ...
        all_rows.append(metric_row(run_id, fault_type, "baseline_1_raw_threshold", p1, r1, f11, mdd1, fab1, 3.0))
        ...
        all_rows.append(metric_row(run_id, fault_type, "baseline_2_residual_threshold", p2, r2, f12, mdd2, fab2, best_threshold_b2))
        ...
        all_rows.append(metric_row(run_id, fault_type, "baseline_3_ml_reconstruction", p3, r3, f13, mdd3, fab3, thr_ml))

    out_df = pd.DataFrame(all_rows)
    out_df.to_csv(root / "data" / "runs" / "week07_ablation_results.csv", index=False)
