...
def metric_row(run_id, fault_type, magnitude, duration, method, precision, recall, f1, mdd, fab, threshold):
    return {
        "run_id": run_id,
...
def main():
    ...
    for _, meta in idx.iterrows():
        ...
        print(f"\nEvaluating {run_id}: {fault_type}, {magnitude}, {duration}")
        ...
        all_rows.append(metric_row(run_id, fault_type, magnitude, duration, "baseline_1_raw_threshold", p1, r1, f11, mdd1, fab1, 3.0))
        ...
        all_rows.append(metric_row(run_id, fault_type, magnitude, duration, "baseline_2_residual_threshold", p2, r2, f12, mdd2, fab2, best_threshold_b2))

        threshold_tuning_rows.append({
            "run_id": run_id,
            "Method": "baseline_2_residual_threshold",
            "val_threshold": best_threshold_b2,
            "test_fab": fab2
        })
        ...
        thr_ml, tuning_records = choose_threshold_on_validation(
            train_score_wide,
            val_score_wide,
        ...
        all_rows.append(metric_row(run_id, fault_type, magnitude, duration, "baseline_3_ml_reconstruction", p3, r3, f13, mdd3, fab3, thr_ml))

        threshold_tuning_rows.append({
            "run_id": run_id,
            "Method": "baseline_3_ml_reconstruction",
            "val_threshold": thr_ml,
            "test_fab": fab3
        })

    out_df = pd.DataFrame(all_rows)
    out_df.to_csv(root / "data" / "runs" / "week08_grid_results.csv", index=False)

    pd.DataFrame(threshold_tuning_rows).to_csv(
        root / "data" / "runs" / "week08_threshold_tuning.csv", index=False
    )
