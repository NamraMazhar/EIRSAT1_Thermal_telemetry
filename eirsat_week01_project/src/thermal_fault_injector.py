...
def apply_fault(df, idxs, fault_type, magnitude, event_seed):
    ...
    if fault_type == "bias":
        ...
        modified = apply_drift(original, magnitude)
...
def main():
    ...
    cfg = load_yaml(root / "configs" / "injection.yaml")
    run_id = cfg["run_id"]
    ...
    for channel in cfg["allowed_channels"]:
        ...
        used_ranges = []
        base_rng = np.random.default_rng(global_seed + abs(hash(channel)) % 100000)
        ...
        for ev_num in range(events_per_channel):
            ...
            apply_fault(df_inj, event_indices, fault_type, float(magnitude), event_seed)
    ...
    for ev in events:
        ...
        assert ev_start >= split_start, f"Event starts before split start: {ev['event_id']}"
        assert ev_end < split_end, f"Event ends after split end: {ev['event_id']}"
